            ...
            <div class="card overflow-hidden">
              <div class="card-body">
                <div class="d-flex justify-content-between align-items-baseline mb-4 mb-md-3 pb-2 border-bottom">
                    ...
                    @endif
                 <h4 class=" mb-0">Football Game List</h4>
                  <div class="d-flex align-items-center flex-wrap text-nowrap">
                  <input type="hidden" name="type" id="type" value="p_l">
                  <form method="GET" action="{{ route('admin-football-game-list') }}">
                  <input class="form-control event-search" id="search" name="search" value="{{ request('search') }}" type="text" placeholder="search">
                        <!--<button type="submit" class="btn btn-primary">Search</button>-->
                         <button type="submit" class="btn btn-primary Refresh btn-icon-text mb-2 mb-md-0">
                     Search
                    </button> 
                    </form>  
                          ...
                          @foreach ($footballs as $d)
                            <tr>
                                ...
                                <td><button class="badge badge-primary edit_btn" style="background-color:rgb(68, 97, 242);" data-match-id={{$d->id}} data-channel-id={{$d->channel_id}}>Edit</button>
                                    <a href="{{route('delete.football.game.new',$d->id)}}" class="badge badge-danger">Delete</a>
                                    </td>
                                <td><a href="/admin/football/match-create/{{$d->id}}" @if($d->status==1) style="color:orange !important;" @else style="color:red !important;" @endif >{{ $d->game_title }}</a></td>
                                <td>{{$d->run_date_time}}</td>
                                <td>{{ $d->status==0? 'Inactive' : 'Active' }}</td>
                                @if($d->status===0)
                                    <td><a href="{{route('activate.football.game.status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(68, 97, 242);">Activate</a></td>
                                @else
                                        <td><a href="{{route('deactivate.football.game.status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(240, 36, 36);">Deactivate</a></td>
                                @endif
                            </tr>
                          @endforeach
                        </tbody>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="editForm" action="{{ route('update.football.game.admin') }}" method="POST">
            @csrf
