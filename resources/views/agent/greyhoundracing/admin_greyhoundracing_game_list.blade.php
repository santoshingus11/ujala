            ...
            <div class="card overflow-hidden">
              <div class="card-body">
                <div class="d-flex justify-content-between align-items-baseline mb-4 mb-md-3 pb-2 border-bottom">
                    ...
                    @endif
                 <h4 class=" mb-0">Grey Hound Racing Game List</h4>
                            ...
                            <tr>
                                <th scope="row">{{$loop->index+1}}</th>
                                 <td><button class="badge badge-primary edit_btn" style="background-color:rgb(68, 97, 242);" data-match-id={{$d->id}} data-channel-id={{$d->channel_id}}>Edit</button>
                                    <a href="{{route('delete.greyhoundracing.game.new',$d->id)}}" class="badge badge-danger">Delete</a>
                                    </td>
                                <td><a href="{{url('/admin/greyhoundracing/match-create/'.$d->id)}}" @if($d->status==1) style="color:orange !important;" @else style="color:red !important;" @endif >{{ $d->game_title }}</a></td>
                                ...
                                @if($d->status===0)
                                    <td><a href="{{route('activate.greyhoundracing.game.status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(68, 97, 242);">Activate</a></td>
                                @else
                                        <td><a href="{{route('deactivate.greyhoundracing.game.status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(240, 36, 36);">Deactivate</a></td>
                                @endif
                            </tr>
                          @endforeach
                        </tbody>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="editForm" action="{{ route('update.greyhoundracing.game.admin') }}" method="POST">
            @csrf
