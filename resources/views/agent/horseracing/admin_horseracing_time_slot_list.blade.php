            ...
            <div class="card overflow-hidden">
              <div class="card-body">
                <div class="d-flex justify-content-between align-items-baseline mb-4 mb-md-3 pb-2 border-bottom">
                    ...
                    @endif
                 <h4 class=" mb-0">Horse Racing Time Slot List</h4>
                        ...
                        <tbody>
                          @foreach ($horseracings as $d)
                            <tr>
                                ...
                                <td><a href="/admin/horseracing/match-create/{{$d->id}}" @if($d->status==1) style="color:orange !important;" @else style="color:red !important;" @endif >{{ $d->time_slot }}</a></td>
                                <td>{{$d->game->game_title}}</td>
                                <td>{{$d->created_at}}</td>
                                <td>{{ $d->status==0? 'Inactive' : 'Active' }}</td>
                                @if($d->status===0)
                                    <td><a href="{{route('activate.horseracing.timeslot.status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(68, 97, 242);">Activate</a></td>
                                @else
                                        <td><a href="{{route('deactivate.horseracing.timeslot.status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(240, 36, 36);">Deactivate</a></td>
                                @endif
                            </tr>
                          @endforeach
                        </tbody>
