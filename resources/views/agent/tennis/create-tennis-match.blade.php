...
<h4 class=" mb-0">Live score</h4>
            <form action="{{route('admin-tennis-match-submit-score')}}" id="" enctype="multipart/form-data" method="POST">
                        @csrf
                        <div class="row mb-4">
            ...
            <div class="card overflow-hidden">
              <div class="card-body">
                <div class="d-flex justify-content-between align-items-baseline mb-4 mb-md-3 pb-2 border-bottom">
                 ...
                 <h4 class=" mb-0">Create Tennis Match</h4>
                  ...
                  <input class="form-control event-search" id="url"  value="{{url('/')}}" type="hidden" placeholder="Event">
                    ...
                    <form action="{{route('admin-tennis-match-submit')}}" id="" enctype="multipart/form-data" method="POST">
                        @csrf
                        {{method_field('POST')}}
                        <div class="row mb-4">
                            <div class="col-lg-12 col-md-12 col-sm-12">
            ...
            <div class="row">
              <div class="col-12 col-xl-12 grid-margin stretch-card">
                <div class="card overflow-hidden">
                  <div class="card-body">
                    <div class="d-flex justify-content-between align-items-baseline mb-4 mb-md-3 pb-2 border-bottom">
                     <h4 class=" mb-0">Tennis Match List</h4>
                      <div class="d-flex align-items-center flex-wrap text-nowrap">
                      <input type="hidden" name="type" id="type" value="p_l">
                      <input class="form-control event-search" id="url"  value="{{url('/')}}" type="hidden" placeholder="Event">
                              ...
                              @foreach ($tennises as $d)
                                <tr>
                                    ...
                                    <td><button class="badge badge-primary edit_btn" style="background-color:rgb(68, 97, 242);" data-match-id={{$d->id}} data-team-name='{{$d->team_name}}' data-back-value={{$d->back_value}} data-lay-value={{$d->lay_value}} data-match-type={{$d->match_type}} >Edit</button>
                                        <a href="{{route('delete.tennis.match',$d->id)}}" class="badge badge-danger">Delete</a>
                                    </td>
                                      <td> 
                                       <a href="{{route('show.cricket.match',['id' => $d->id, 'domain' => 'newsilver.art', 'game' => 'tennis'])}}" class="badge badge-eye"> <img src="https://newsilver.art/public/assets/logologin.png" alt="Profile Picture 1"></a>
                                       <a href="{{route('show.cricket.match',['id' => $d->id, 'domain' => 'allpanel.art', 'game' => 'tennis'])}}" class="badge badge-eye"> <img src="https://allpanel.art/public/login/logo.png" alt="Profile Picture 1"></a>
                                       <a href="{{route('show.cricket.match',['id' => $d->id, 'domain' => 'crickekbuz.art', 'game' => 'tennis'])}}" class="badge badge-eye"> <img src="https://crickekbuz.art/public/login/logo.png" alt="Profile Picture 1"></a>
                                       <a href="{{route('show.cricket.match',['id' => $d->id, 'domain' => 'laserclub.art', 'game' => 'tennis'])}}" class="badge badge-eye"> <img src="https://laserclub.art/public/assets/logo.gif" alt="Profile Picture 1"></a>
                                       <a href="{{route('show.cricket.match',['id' => $d->id, 'domain' => 'gold365.art', 'game' => 'tennis'])}}" class="badge badge-eye"> <img src="https://gold365.art/public/frontend/assets/images/logo-login.png" alt="Profile Picture 1"></a>
                                    </td>
                                    <td><a href="/admin/tennis/change-match-status/{{$d->id}}/{{$d->game_id}}" style=@if(isset($d->win_loss))  {{"color:red;"}} @else {{"color:blue;"}} @endif</a>{{ $d->team_name }}</a></td>
                                    <td>{{ $d->back_value }}</td>
                                    <td>{{ $d->lay_value }}</td>
                                    <td>{{ $d->status==0? 'Inactive' : 'Active' }}</td>
                                    @if($d->back_status===0)
                                    <td><a href="{{route('tennis.activate.back_status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(68, 97, 242);">Activate</a></td>
                                    @else
                                        <td><a href="{{route('tennis.deactivate.back_status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(240, 36, 36);">Deactivate</a></td>
                                    @endif
                                    
                                    @if($d->lay_status===0)
                                    <td><a href="{{route('tennis.activate.lay_status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(68, 97, 242);">Activate</a></td>
                                    @else
                                        <td><a href="{{route('tennis.deactivate.lay_status',$d->id)}}" class="badge badge-primary" style="background-color:rgb(240, 36, 36);">Deactivate</a></td>
                                    @endif
...
console.log($(this).data("team-name"));
      ...
      $('#create_match_form').attr('action', "/admin/tennis/match-update")
